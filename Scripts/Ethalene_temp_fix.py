...
# -----------------------------
velocity = 950.0  # m/s (start subsonic)
length = 0.7  # m
dx = 0.001  # m
n_steps = int(length / dx)
dt = dx / velocity  # time step
C2H4_injection_point = 0.05  # m
...
for i in range(n_steps):
    ...
    if distance >= C2H4_injection_point:
        mix_fraction = min((distance - C2H4_injection_point) / injection_length, 1.0)
        O2 = 2.0 * (1 - mix_fraction)
        N2 = 7.52 * (1 - mix_fraction)
        C2H4 = 1.0 * mix_fraction
        new_comp = f'C2H4:{C2H4}, O2:{O2}, N2:{N2}'
    ...
    if not ignited and X[gas.species_index('C2H4')] < 0.9 and T - initial_temperature > 100:
        ignited = True
        ignition_position = distance

    if post_combustion_index is None and X[gas.species_index('C2H4')] < 1e-4:
...
gas_equil.TPX = initial_temperature, initial_pressure, 'C2H4:1, O2:2, N2:7.52'
...
print(f" C₂H₄ Conversion: {C2H4_conversion:.2f}%")
...
axs[0, 0].axvline(C2H4_injection_point, color='black', linestyle='--', label='C₂H₄ Injected')
...
plt.axvline(C2H4_injection_point, color='black', linestyle='--', label='C₂H₄ Injected')
